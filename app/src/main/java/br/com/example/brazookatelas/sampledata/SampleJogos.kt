package br.com.example.brazookatelas.sampledata

import br.com.example.brazookatelas.model.Jogos


val sampleJogos = listOf(

    Jogos(
        nome = "99Vidas",
        genero = "Ação",
        classificacao = "12",
        estudio = "QUByte Interactive Ltda.",
        sinopse = "\"99Vidas - O Jogo\" traz de volta o prazer de jogar games no estilo beat'em up como Final Fight, Double Dragon, Battletoads, Golden Axe e Streets of Rage, remetendo aos estilo retro pixel-art 16bits e trazendo toda a nostalgia desse grande gênero que dominou os arcades e consoles na década de 90.",
        ano = 2016,
        plataformas = "PC, Nintendo Switch, Playstation 3, Xbox One",
        capa = "https://cdn.mobygames.com/3ddb508e-ab77-11ed-8d2b-02420a00019e.webp"
    ),

    Jogos(
        nome = "Horizon Chase Turbo",
        genero = "Ação",
        classificacao = "12",
        estudio = "Aquiris",
        sinopse = "Horizon Chase Turbo é uma corrida ao redor do mundo. A cada nova copa, você pilotará seu carro por lugares extraordinários, observando o pôr do sol, enfrentando chuva, neve, cinzas vulcânicas e até fortes tempestades de areia. Seja de dia ou de noite cada trilha se passa em lindos cartões postais do mundo todo.",
        ano = 2018,
        plataformas = "PC, PS4, Xbox One e Switch",
        capa = "https://cdn.mobygames.com/f236aed6-ab8e-11ed-b042-02420a00019f.webp",
    ),

    Jogos(
        nome = "Chroma Squad",
        genero = "Ação",
        classificacao = "12",
        estudio = "Behold Studios",
        sinopse = "Chroma Squad é um RPG tático sobre cinco dublês que decidem sair de seus trabalhos e criar o seu próprio estúdio de TV inspirado em Power Rangers! Contrate atores, compre equipamentos e melhorias para o seu estúdio, crie armas e robôs gigantes feitos de papelão e fita crepe.",
        ano = 2015,
        plataformas = "PC, Xbox One, PS4, Switch, Android e iOS.",
        capa = "https://cdn.mobygames.com/5efcf540-ab6a-11ed-9b12-02420a00019d.webp",
    ),

    Jogos(
        nome = "Dandara",
        genero = "Ação",
        classificacao = "12",
        estudio = "Long Hat House",
        sinopse = "Em um universo bizarro onde os oprimidos estão à beira do esquecimento, Dandara despertou para remodelar o mundo.",
        ano = 2018,
        plataformas = "PC, Xbox One, PS4, Switch, Android e iOS",
        capa = "https://cdn.mobygames.com/8b83e98c-ac07-11ed-9899-02420a000134.webp",
    ),

    Jogos(
        nome = "Blazing Chrome",
        genero = "Ação",
        classificacao = "12",
        estudio = "The Arcade Crew",
        sinopse = "Em Blazing Chrome, as máquinas dominam o mundo e os poucos humanos sobreviventes estão à beira do extermínio, sem capacidade para fazerem frente aos seus inimigos metálicos.",
        ano = 2019,
        plataformas = "PC, Xbox One, PS4 e Switch",
        capa = "https://cdn.mobygames.com/60ef4598-abfa-11ed-b075-02420a00012f.webp",
    ),

    Jogos(
        nome = "Minoria",
        genero = "Ação",
        classificacao = "12",
        estudio = "Bombservice",
        sinopse = "Tinta e sangue mancham as ruas de Ramezia. Bruxas e bruxos devem ser queimados e pecados têm que ser julgados. Aventure-se na pele da Irmã Semilla, que tem a missão de reduzir os pilares da heresia às cinzas, nesse jogo de ação em plataforma feito pelos criadores de Momodora, Bombservice.",
        ano = 2019,
        plataformas = "PC, Xbox One, PS4 e Switch",
        capa = "https://cdn.mobygames.com/5f18235e-ac07-11ed-b85a-02420a000135.webp",
    ),

    Jogos(
        nome = "Knights of Pen and Paper",
        genero = "Ação",
        classificacao = "10",
        estudio = "Behold Studios",
        sinopse = "Prepare-se para habitar um mundo de cavalheirismo, luta entre classes e referências pop fora do comum. Knights of Pen & Paper 2 é uma aventura baseada em turnos, estilo retrô e pixel art cheia de perigo, intriga, morte e jogadas de salvamento!",
        ano = 2012,
        plataformas = "PC,PS4,Xbox One",
        capa = "https://cdn.mobygames.com/f24073a8-ab75-11ed-8bed-02420a00019f.webp",
    ),

    Jogos(
        nome = "A Lenda do Herói",
        genero = "Ação",
        classificacao = "12",
        estudio = "Dumativa",
        sinopse = "Aventure-se no mundo do Herói em pixel art no qual as ações do jogador são cantadas de forma cômica e dinâmica, nesse jogo de plataforma e muita aventura!",
        ano = 2020,
        plataformas = "PC",
        capa = "https://bdjogos.com.br/capas/8551-a-lenda-do-heroi-pc-capa-1.jpg",
    ),

    Jogos(
        nome = "No Heroes Here",
        genero = "Ação",
        classificacao = "10",
        estudio = "Mad Mimic",
        sinopse = "No Heroes Here é um jogo frenético para 1 a 4 jogadores, disponível nos modos local e online. Confeccione e use diversos itens enquanto coordena sua equipe de Não-Heróis para defender castelos e sobreviver! ...Ah, e salvar o reino no meio tempo também!",
        ano = 2017,
        plataformas = " PC, PS4, Xbox One, Switch e Android",
        capa = "https://cdn.mobygames.com/fd0f228e-ab9d-11ed-b9d3-02420a00019b.webp",
    ),

    Jogos(
        nome = "Out There Somewhere",
        genero = "Ação",
        classificacao = "10",
        estudio = "MiniBoss",
        sinopse = "Inspirado por Cave Story, Super Metroid, e Portal, você joga como um explorador encalhado em um perigoso planeta alienígena. Com pouco combustível, a sua única chance de escapar é encontrar o núcleo do planeta nesse incrivelmente inteligente jogo de plataformas de ação e raciocínio.",
        ano = 2016,
        plataformas = "PC",
        capa = "https://cdn.mobygames.com/b67cebec-ab6d-11ed-9d05-02420a000198.webp",
    ),

    Jogos(
        nome = "Monica e a Guarda dos Coelhos",
        genero = "Ação",
        classificacao = "L",
        estudio = "Mad Mimic Interactive",
        sinopse = "MÔNICA E A GUARDA DOS COELHOS é um jogo cooperativo local, de um a quatro jogadores. Controlando um grupo de heróis da Turma da Mônica, todo mundo precisará exercer um papel e defender castelos!",
        ano = 2018,
        plataformas = "Xbox, PS4, Nitendo Switch, PC",
        capa = "https://cdn.mobygames.com/6f65ccd2-abf5-11ed-b7c8-02420a000136.webp",
    ),

    Jogos(
        nome = "Momodora: Reverie Under The Moonlight",
        genero = "Ação",
        classificacao = "12",
        estudio = "Bombservice",
        sinopse = "Reverie Under the Moonlight é o quarto jogo na amada série de jogos de plataforma 2D, Momodora. Realize combos e esquive de seus inimigos para fazer seu caminho através de uma terra amaldiçoada e se encontrar com a Rainha, para enfim acabar com o mal que ameaça todas as formas de vida.",
        ano = 2016,
        plataformas = "Xbox, PS4, Nitendo Switch, PC",
        capa = "https://cdn.mobygames.com/39c869f0-ab7c-11ed-ba50-02420a000199.webp",
    ),

    Jogos(
        nome = "Damned",
        genero = "Terror",
        classificacao = "12",
        estudio = "9heads Game Studios",
        sinopse = "Damned é um jogo de terror online aleatório para até 5 jogadores (4 sobreviventes e 1 monstro). Você poderá jogar como uma das quatro pessoas presas em diferentes lugares mal-assombrados (hotéis, hospitais, etc).",
        ano = 2014,
        plataformas = "PC",
        capa = "https://cdn.mobygames.com/a875a4d8-ab6f-11ed-99ae-02420a00019e.webp",
    ),

    Jogos(
        nome = "Colina: Legacy",
        genero = "Terror",
        classificacao = "14",
        estudio = "Chance6 Studios, LLC",
        sinopse = "Alex acorda na casa de sua avó, sozinho e sem saber onde estão todos, ele começa a fugir, a sensação de alguém ... ou algo que o observa em sua mente. Diante do desconhecido, Alex precisa passar a noite na casa antes que ele perca mais do que apenas sua mente.",
        ano = 2018,
        plataformas = "Xbox, PS4, Nitendo Switch, PC",
        capa = "https://cdn.mobygames.com/edeb9f22-abfb-11ed-9c12-02420a00012f.webp",
    ),

    Jogos(
        nome = "Pesadelo: O início",
        genero = "Terror",
        classificacao = "14",
        estudio = "Skyfaz",
        sinopse = "Um jogo de terror para computadores com Windows feito por brasileiros e ambientado nas estações de metrô de São Paulo. O título é mais um da nova onda de games do gênero inspirados em Slender.",
        ano = 2013,
        plataformas = "PC",
        capa = "https://3.bp.blogspot.com/-kq4w3Lr2sf4/UWsxdFkytmI/AAAAAAAACqU/hl_Apx6v1C8/s280/429508_525461924161916_1232080159_n.jpg",
    ),

    Jogos(
        nome = "Insanidade",
        genero = "Terror",
        classificacao = "14",
        estudio = "Simperson",
        sinopse = "Insanidade é um jogo em primeira pessoa do gênero horror onde o jogador terá que ajudar o personagem principal a desvendar alguns mistérios, recuperar suas memórias encontrando algumas fitas K7 pelo cenário e se protegendo de possíveis ameaças escondidas pela fazenda.",
        ano = 2013,
        plataformas = "PC",
        capa = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTfKEn6i-adrRjqDYe_9hM7i3l9J83-6JN8qQ69g6g6vapkJGli0GfiOeSMI6vL47sgHLg&usqp=CAU",
    ),

    Jogos(
        nome = "Fobia – St. Dinfna Hotel",
        genero = "Terror",
        classificacao = "14",
        estudio = "Pulsatrix",
        sinopse = "O protagonista é um jornalista recém-formado que está à procura de um furo de reportagem sobre o fundador do hotel, foi assassinado misteriosamente no passado. Só que, como era de se esperar, nada sai como ele esperava – e muitos mistérios macabros surgem.",
        ano = 2022,
        plataformas = "PC, PlayStation, Xbox",
        capa = "https://cdn.mobygames.com/f8cab83a-ac12-11ed-aeca-02420a000130.webp",
    ),

    Jogos(
        nome = "Until Dead",
        genero = "Terror",
        classificacao = "12",
        estudio = "Monomyto Game",
        sinopse = "Until Dead é um jogo gratuito de terror, que apresenta estética monocromática, inspiração em filmes noir e é ambientado em um universo pós-apocalíptico com zumbis.",
        ano = 2017,
        plataformas = "Android, iOS",
        capa = "https://is1-ssl.mzstatic.com/image/thumb/Purple124/v4/20/d6/08/20d6089c-23fc-8bb2-34c5-2a3638a298fc/pr_source.png/643x0w.jpg",
    ),

    Jogos(
        nome = "ENEFN",
        genero = "Terror",
        classificacao = "12",
        estudio = "Theago Liddell",
        sinopse = "Com estética retrô em pixel art, ENEFN é um jogo de exploração com elementos de survival horror, que coloca o jogador como um aluno que ficou preso em uma versão assombrada de sua escola.",
        ano = 2020,
        plataformas = "PC, Android",
        capa = "https://img.youtube.com/vi/FcK01HpIZEs/0.jpg",
    ),

    Jogos(
        nome = "D3L3T3.exe",
        genero = "Terror",
        classificacao = "12",
        estudio = "Labbitspace",
        sinopse = "Numa corrida contra o tempo, um jovem percorre um cemitério no meio do nada, em busca de algo que possa quebrar uma maldição fatal. Dentre assassinatos e segredos ocultos, apenas a escuridão é uma certeza.",
        ano = 2021,
        plataformas = "PC",
        capa = "https://cdn.akamai.steamstatic.com/steamcommunity/public/images/apps/1546950/e56024a822814d8402834d69d199d080b6248290.jpg",
    ),

    Jogos(
        nome = "Devil Inside Us: Roots of Evil",
        genero = "Terror",
        classificacao = "12",
        estudio = "Mr.Skull Game Studio",
        sinopse = "Jogue como o exorcista Aughust Heylel! Utilize seu crucifixo e habilidades mediúnicas para expurgar o mal nessa experiência de terror em primeira pessoa!",
        ano = 2021,
        plataformas = "PC",
        capa = "https://cdn.cloudflare.steamstatic.com/steam/apps/1543520/header.jpg?t=1635390168",
    ),

    Jogos(
        nome = "No Place for Bravery",
        genero = "Aventura",
        classificacao = "10",
        estudio = "Glitch Factory",
        sinopse = "Thorn, um velho e ambicioso guerreiro, perambula por um mundo devastado pela guerra em busca de sua filha perdida. Sobreviva às batalhas desenfreadas e violentas neste RPG de ação 2D com visão de cima, e descubra a história arrepiante por trás da odisseia de Thorn.",
        ano = 2022,
        plataformas = "PC, Nintendo Switch",
        capa = "https://cdn.mobygames.com/7291ad26-ac14-11ed-a599-02420a000132.webp",
    ),

    Jogos(
        nome = "Horizon Chase 2",
        genero = "Ação",
        classificacao = "L",
        estudio = "Aquiris Game Studio",
        sinopse = "A sequência do aclamado Horizon Chase, que repete a fórmula de jogo de corrida com inspiração nos clássicos do gênero dos anos 1980 e 1990, como Top Gear.",
        ano = 2022,
        plataformas = "Mobile",
        capa = "https://cdn.mobygames.com/189d4942-ac14-11ed-8ce3-02420a000131.webp",
    ),

    Jogos(
        nome = "Hazel Sky",
        genero = "Aventura",
        classificacao = "L",
        estudio = "Coffee Addict Studio",
        sinopse = "Hazel Sky é uma história comovente sobre um jovem engenheiro desafiando seu destino e seus desejos. Conserte máquinas, pule, escale, balance e escorregue por um belo e misterioso mundo.",
        ano = 2022,
        plataformas = "PlayStation 4, Xbox One, Nintendo Switch, PC",
        capa = "https://cdn.mobygames.com/6c2d81d0-ac14-11ed-8ce3-02420a000131.webp",
    ),

    Jogos(
        nome = "Kukoos: Lost Pets",
        genero = "Aventura",
        classificacao = "L",
        estudio = "PetitFabrik",
        sinopse = "Descubra o universo dos Kukoos neste incrível jogo de plataforma 3D. Explore mundos misteriosos e descubra mistérios para libertar os pets que sofreram lavagem cerebral. Conheça grandes personagens, novas mecânicas, lugares malucos e divirta-se a valer!",
        ano = 2021,
        plataformas = "PlayStation 4, Nintendo Switch e PC",
        capa = "https://cdn.mobygames.com/d7aa030a-ac16-11ed-8d66-02420a000132.webp",
    ),

    Jogos(
        nome = "Togges",
        genero = "Ação",
        classificacao = "L",
        estudio = "Regular Studio",
        sinopse = "Togges é um Plataforma 3D collect-a-thon sobre espalhar e empilhar adoráveis cubos ao redor do espaço-sideral! Explore mundos únicos, solucione enigmas, e supere desafios nessa aventura rumo a dominar o universo!",
        ano = 2022,
        plataformas = "PlayStation 4, PlayStation 5, Xbox One, Xbox Series X|S, Nintendo Switch, PC",
        capa = "https://cdn.mobygames.com/6d930656-ac16-11ed-9271-02420a000133.webp",
    ),

    Jogos(
        nome = "Esquadrão 51 contra os Discos Voadores",
        genero = "",
        classificacao = "12",
        estudio = "Loomiarts e Fehorama Filmes",
        sinopse = "OVNIS! Alienígenas! Aviões! Resistência! SHOOT’EM UP! Esquadrão 51 é um shooter espacial com jogabilidade 2D e ambientação 3D. Divirta-se com a incrível combinação de uma acelerada experiência de tiro e a estética dos filmes de ficção científica dos anos 50,",
        ano = 2022,
        plataformas = "PC",
        capa = "https://cdn.mobygames.com/78b41bd0-c8ca-11ed-a401-02420a00016a.webp",
    ),

    Jogos(
        nome = "Relic Hunters: Rebeldes",
        genero = "Aventura",
        classificacao = "L",
        estudio = "Rogue Snail",
        sinopse = "Rápido. Espontâneo. Divertido. Junte até 3 amigos, equipe-se e salve a Galáxia de um terrível vilão que roubou o passado neste jogo on-line cooperativo de RPG de ação looter-shooter.",
        ano = 2023,
        plataformas = "Mobile",
        capa = "https://cdn.mobygames.com/1a6ecea4-ac04-11ed-902e-02420a00012d.webp",
    ),

    Jogos(
        nome = "Pãozito",
        genero = "Casual",
        classificacao = "L",
        estudio = "Magalu Games",
        sinopse = "O jogador controla um pedaço de pão, que precisa salvar o café da manhã! Para isso, é preciso ser arremessado por uma torradeira na direção de balões que, quando estourados, geram bacon, leite, ovos e outras comidas para a refeição perfeita.",
        ano = 2022,
        plataformas = "Mobile",
        capa = "https://play-lh.googleusercontent.com/O5o3lF_5z1ULs0wl5H0Aaj5XXpJFvwxITpDicV4oZUYSsqu7_yriJY_vDD1TGCEhce5W=w526-h296-rw",
    ),

    Jogos(
        nome = "Dolmen",
        genero = "Ação",
        classificacao = "12",
        estudio = "Massive Work Studio",
        sinopse = "Dolmen é um jogo de ação em terceira pessoa com elementos de RPG, onde Sci-Fi e Cosmic Horror são misturados em uma experiência de jogo envolvente! Pegue a sua chave no Kickstarter!",
        ano = 2022,
        plataformas = "PlayStation 4, PlayStation 5, Xbox One, Xbox Series X|S, PC",
        capa = "https://cdn.mobygames.com/d034c104-ac12-11ed-aca6-02420a000132.webp",
    ),

    Jogos(
        nome = "171",
        genero = "Ação",
        classificacao = "18",
        estudio = "Betagames Group",
        sinopse = "171 é um jogo de ação e aventura de mundo aberto com ambientação inspirada no Brasil.",
        ano = 2022,
        plataformas = "PC",
        capa = "https://gamerview.uai.com.br/wp-content/uploads/2023/01/171-header.jpg",
    ),

    Jogos(
        nome = "Arida: Backland's Awakening",
        genero = "Aventura",
        classificacao = "L",
        estudio = "Aoca Game",
        sinopse = "Arida narra a história de Cícera, uma jovem sertaneja que inicia uma jornada pelo sertão brasileiro no século XIX. O jogo é influenciado por elementos da cultura e da sabedoria popular.",
        ano = 2019,
        plataformas = "PC, Mobile",
        capa = "https://store-images.s-microsoft.com/image/apps.4801.14213541312509668.491f1de9-ea1e-4cc9-969b-35354172484b.cf186198-c4b6-4086-b466-36bdfdd28d4c?mode=scale&q=90&h=225&w=150",
    ),
)

val sampleSectionsJogos = mapOf(
    "Em alta" to sampleJogos,
    "Recomendados" to sampleJogos,
    "Outros" to sampleJogos
)


